...
# Streamlit app
def main():
    st.title("YouTube Video to Sign Language Converter")
    ...
    if st.button("Process"):
        if youtube_url:
            ...
            # Transcription
            trans_status.text("Transcribing video...")
            processor = VideoProcessor()
            try:
                transcript = processor.process_video(youtube_url)
                trans_progress.progress(100)
                trans_status.success("Transcription complete!")
                st.subheader("Transcript:")
                st.write(transcript)

                # Mapping to sign language
                mapping_status.text("Mapping to sign language...")
                text_to_vid(transcript, chunk_size=500)
                mapping_progress.progress(100)
                mapping_status.success("Mapping complete!")
